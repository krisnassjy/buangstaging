...
const filterAll = async () => {
  ...
  containerSearch = homeData;

  containerSearch.map((item) => {
    appendCard(item.src);
  });
...
const filterDark = async () => {
  await setIsLoading(true);
  containerSearch = homeData.filter((item) => item.category === "dark");
...
const filterLight = async () => {
  await setIsLoading(true);

  containerSearch = homeData.filter((item) => item.category === "light");
...
const filterColourful = async () => {
  await setIsLoading(true);

  containerSearch = homeData.filter((item) => item.category === "colourful");
...
const search = async (event) => {
  await setIsLoading(true);
  containerSearch = homeData.filter((item) => item.name.toLowerCase().includes(event.target.value))
...
url === "index.html" ? homeRender() : categoryRender();
